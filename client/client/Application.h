...
{
public:
	Application(HWND window, int width, int height, int canvasWidth, int canvasHeight)
	{
	...
	void Initialize(HWND window, int width, int height, int canvasWidth, int canvasHeight)
	{
		...
		// CreateRenderTarget
		ID3D11Texture2D* pBackBuffer; // 실제 그림이 그려지는 곳
		...
		// Set the viewport
		// 뷰포트 : 윈도우 내에서 얼마정도를 그림 그릴곳으로 쓸거냐
